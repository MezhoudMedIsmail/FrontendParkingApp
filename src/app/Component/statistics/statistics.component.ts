...
constructor( private placeService : PlaceParkingService,private userService : UserServiceService){
    ...
    series: [
      {
        ...
        data: [
          {
            value: this.placeParkingList.length,
      ...
      },
      {
        ...
        data: [
          {
            value: this.placeParkingList.filter(obj => obj.status === false ).length,
            symbol: pathSymbols.car
          }
        ]
      }
...
getHistory() {
  this.placeService.getHistory().subscribe((res: any[]) => {
    ...
    forkJoin(observables).subscribe(updatedElements => {
      // updatedElements contains the elements with the username information added
